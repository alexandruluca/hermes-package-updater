...
class Updater extends EventEmitter {
	...
	canInstallDeployment(deployment, options = {}) {
		...
		if (config.band === Band.PRODUCTION) {
			if (deployment.serverTags && deployment.serverTags.includes(config.serverTag)) {
		...
		if(deployment.band === Band.QA) {
			...
			if (options.isNewDeployment) {
				if (monitoredApp.pullRequestMeta) {
					...
					let monitoredAppIssueNumber = monitoredApp.pullRequestMeta.issueNumber;

					if (monitoredApp.pullRequestMeta.issueNumber === deployment.pullRequestMeta.issueNumber) {
						logger.info(`deployment ${deploymentIdentifier} is installable, same issueNumber deployment was identified`);
						return true;
					} else {
						let message = `deployment ${deploymentIdentifier} is not installable, new deployment is based of ${issueNumber}` +
							`, monitored app issue number is ${monitoredAppIssueNumber}`;
