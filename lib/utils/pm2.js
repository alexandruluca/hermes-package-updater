...
async function pm2Reload(appName) {
	...
	let {runtimeOptions, configurable} = getManifest(watchedApp.projectPath);
	...
	if (configurable) {
		let projectConfig = await getProjectConfiguration(config.serverTag, name);

		let configPath = path.join(config.configDir, name, 'config.json');
	...
	if (!isStarted) {
		await startApp({...watchedApp, runtimeOptions});
		return 'started';
	}
