...
class Application {
	...
	doUpdate() {
		...
		let availableDeployment = this.getAvailableDeployment();
		let {pullRequestMeta, ...deployment} = availableDeployment;
		let isIncomingPullRequest = !!pullRequestMeta;

		this.setUpdateAvailable(false);

		let message = `Preparing to install ${utils.deploymentToString(availableDeployment)}`;
		logger.info(message);
